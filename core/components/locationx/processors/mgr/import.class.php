...
class lxStoreImportProcessor extends modProcessor {
    ...
    public function prepare() {
        ...
        if (!empty($f['name']) && ($f['size'] > 0) && ($f['error'] == 0) && !empty($f['tmp_name'])) {
            ...
            $this->modx->log(modX::LOG_LEVEL_INFO,$this->modx->lexicon('locationx.import.file_received',array('size' => $size)));
            return true;
        } else {
            return false;
        }
    ...
    public function parseXML() {
        $this->modx->log(modX::LOG_LEVEL_INFO,$this->modx->lexicon('locationx.import.parsing_xml'));

        if (!function_exists('simplexml_load_string')) {
            $this->modx->log(modX::LOG_LEVEL_ERROR,$this->modx->lexicon('locationx.import.missing_simplexml'));
            return false;
        }
        $this->xml = simplexml_load_string($this->raw,'SimpleXMLElement');
        if (!$this->xml) {
            $this->modx->log(modX::LOG_LEVEL_ERROR,$this->modx->lexicon('locationx.import.error_parsing_xml'));
            return false;
        }

        if (!isset($this->xml->lxStore)) {
            $this->modx->log(modX::LOG_LEVEL_ERROR,$this->modx->lexicon('locationx.import.error_xml_misses_lxstore'));
            return false;
        }

        /* Start looping */
        $total = count($this->xml->lxStore);
        $steps = ceil($total / 15);
        $i = 0;
        ...
        sleep (2);
        foreach ($this->xml->lxStore as $store) {
            $i++;
    ...
    private function importStore($store) {
        ...
        if ($this->overwriteStores) {
            $o = $this->modx->getObject('lxStore',(int)$store->id);
        }
        if (!$o) {
            $o = $this->modx->newObject('lxStore');
            $o->set('category', -1);
            $o->set('gmap_marker', -1);
            $o->set('active', true);
        }
    ...
    public function getCategoryByName($name) {
        ...
        /* @var lxCategory $o */
        $o = $this->modx->getObject('lxCategory',array('name' => $name));
        if (!$o && $this->autoCreateCategories) {
            $o = $this->modx->newObject('lxCategory', array('name' => $name, 'visible' => true));
            $o->save();
        }
        if ($o) {
    ...
    private function getLatLong(lxStore $o) {
        ...
        $result = $this->modx->runProcessor('rest/geocode',array(
            ...
            'processors_path' => $this->modx->locationx->config['processors_path']
        ));
