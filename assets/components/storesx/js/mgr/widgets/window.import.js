StoresX.window.Import = function(config) {
    config = config || {};
    this.id = config.id = config.id || Ext.id();
    config.register = config.register || 'mgr';
    config.topic = config.topic || '/storesx/import/' + this.id + '/';
    Ext.applyIf(config,{
        title: _('storesx.import'),
        ...
        fields: [{
            ...
            xtype: 'textfield',
            fieldLabel: _('storesx.xmlfile'),
            name: 'xmlfile',
            inputType: 'file'
        },{
            xtype: 'checkbox',
            boxLabel: _('storesx.overwrite_stores'),
            name: 'overwrite_stores'
        },{
            xtype: 'checkbox',
            boxLabel: _('storesx.auto_create_categories'),
            name: 'auto_create_categories',
            checked: true
        },{
            xtype: 'checkbox',
            boxLabel: _('storesx.import.get_latlong'),
            name: 'get_latlong',
            checked: false,
            disabled: true,
            description: _('storesx.import.get_latlong.disabled_quota')
        },{
            xtype: 'panel',
            height: 250,
            scrollable: true,
    ...
    StoresX.window.Import.superclass.constructor.call(this,config);
    this.addEvents({
        'shutdown': true,
        'complete': true
    });
...
Ext.extend(StoresX.window.Import,MODx.Window,{
    running: false,
    startImport: function() {
        ...
        body.el.insertHtml('beforeEnd',_('storesx.import.initiated') + '<br />');
    ...
    onComplete: function() {
        ...
        var body = Ext.getCmp(this.id + '-body');
        body.el.insertHtml('beforeEnd',_('storesx.import.done') + '<br />');
...
Ext.reg('storesx-window-import',StoresX.window.Import);
